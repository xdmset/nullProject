...
const MUNDO_ID = 3; // ID para el mundo Jungla

// Rutas de imágenes centralizadas
const IMG_PATHS = {
  flags: {
    activo: "/src/assets/game/icons-jungla/jungla1.png",
    desactivo: "/src/assets/game/icons-jungla/jungla2.png",
    bloqueado: "/src/assets/game/icons-jungla/jungla3.png",
  },
  banana: "/src/assets/icons/banano.png",
  star: "/src/assets/icons/star.png",
  starEmpty: "/src/assets/icons/star-null.png",
  cofres: (level) => `/src/assets/game/cofres/cofre${level}.png`,
  background: "/src/assets/game/icons-jungla/jungle.png",
};
...
export default function JunglaPage() {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const updatedLevels = baseLevels.map((level, index) => ({
          ...level,
          difficulty: index < 4 ? difficultyMap[index] : level.difficulty,
        }));
        ...
        console.error("Error al cargar los datos del mundo Jungla:", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  // ======== Computed values ========
  const nivelesCompletados = progreso?.niveles_completados || 0;
  const shouldAnimateCofre = nivelesCompletados >= (levels.length || 4);

  // ======== Helpers ========
  const isUnlocked = (levelOrder) => levelOrder <= nivelesCompletados + 1;

  const getFlagImage = (levelOrder) => {
    if (levelOrder <= nivelesCompletados) return IMG_PATHS.flags.activo;
    if (isUnlocked(levelOrder)) return IMG_PATHS.flags.desactivo;
    return IMG_PATHS.flags.bloqueado;
  };

  const getCofreImage = () => {
    const cofreLevel = Math.min(nivelesCompletados + 1, 5);
    return IMG_PATHS.cofres(cofreLevel);
  };
  ...
  const renderStars = (levelNumber) => {
    const starsKey = `levelStars-jungla`;
    ...
    return (
      <div className="flex justify-center gap-1 mt-2">
        {[1, 2, 3].map((star, index) => (
          <img
            key={star}
            src={star <= stars ? IMG_PATHS.star : IMG_PATHS.starEmpty}
            alt={star <= stars ? "Estrella ganada" : "Estrella vacía"}
            className={`w-5 h-5 transition-transform duration-300 ${
              star <= stars
                ? "animate-bounce-slow hover:scale-110"
                : "opacity-70"
            }`}
  ...
  const handleCofreClick = () => {
    if (shouldAnimateCofre) setShowReward(true);
  };
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url(${IMG_PATHS.background})` }}
    >
      {/* Botón volver */}
      <button
      ...
      {/* Niveles */}
      <div className="absolute bottom-10 w-full flex gap-32 flex-wrap justify-center items-end">
        {levels.map((level, index) => {
          ...
          return (
            ...
            >
              ...
              />
              <span className="font-bold mt-2 text-xl tracking-wide drop-shadow-sm text-yellow-200">
                {level.nombre}
              </span>

              {/* Tooltip */}
              {unlocked && hoveredLevel === level.id && (
                <div className="absolute bottom-full mb-4 w-72 bg-gradient-to-br from-green-100 to-yellow-100 rounded-3xl p-5 shadow-2xl z-30 animate-fade-in backdrop-blur-md border-4 border-white/50">
                  <h6 className="text-xl font-extrabold text-center drop-shadow-sm">
                    ¡Explora {level.nombre}!
                  </h6>
                  <div className="mt-3 text-center text-base text-green-800 font-medium">
                    <p className="mb-2">
                      Ejercicios:{" "}
                      <span className="font-bold">
                        {level.cantidad_ejercicio}
                      </span>
                      <br />
                      Dificultad:{" "}
                    ...
                    {nivelesCompletados >= levelOrder && (
                      <div>
                        ...
                        <div className="flex justify-center">
                          {renderStars(levelOrder)}
                        </div>
                      </div>
                    )}
                  </div>
                  <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-8 border-r-8 border-t-8 border-l-transparent border-r-transparent border-t-yellow-100"></div>
                </div>
              )}
            </div>
          );
        ...
        >
          <img
            ...
            className={`w-40 h-40 object-contain ${
              shouldAnimateCofre ? "animate-wiggle" : ""
            }`}
          />
        </div>
      </div>
