...
const IMG_PATHS = {
  flags: {
    activo: "/src/assets/game/icons-secrets/activo.png",
    desactivo: "/src/assets/game/icons-secrets/desactivo.png",
    bloqueado: "/src/assets/game/icons-secrets/bloqueado.png",
  },
  ...
  background: "/src/assets/game/icons-secrets/secretLevel.png",
};
...
export default function SecretPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // Tu lógica original de dificultad
                const difficultyMap = [5];
                ...
                console.error("Error al cargar los datos del mundo Secreto:", error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const shouldAnimateCofre = nivelesCompletados >= (levels.length || 1);
    ...
    const renderDifficulty = (difficulty) =>
        Array.from({ length: difficulty }).map((_, i) => (
            <img key={i} src={IMG_PATHS.banana} alt="banana" className="w-6 h-6 inline-block mx-0.5" />
        ));

    const renderStars = (levelNumber) => {
        const starsKey = `levelStars-secret`;
            ...
            <div className="absolute bottom-10 w-full flex gap-32 flex-wrap justify-center items-end">
                {levels.map((level, index) => {
                    ...
                    return (
                        ...
                        >
                            <img src={bandera} alt={`Nivel ${level.nombre}`} className="w-40 h-40 object-contain drop-shadow-lg" />
                            <span className="font-bold mt-2 text-xl tracking-wide drop-shadow-sm text-white">{level.nombre}</span>
